 ...
 * linked to the ordered drug. Checks exemption rules to determine if the line item should be marked
 * as EXEMPTED or PENDING.
 */
@Slf4j
public class DrugOrderBillingStrategy implements OrderBillingStrategy {
	...
	public Optional<Bill> generateBill(Order order) {
		try {
			...
			log.error("Error generating bill for drug order: {}", e.getMessage(), e);
			return Optional.empty();
		}
	}
	
	protected BillLineItem createLineItem(StockItem stockItem, int quantity, BillStatus lineItemStatus, Order order) {
